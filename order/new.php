	...
	if (check_input($_POST, $input_data) == false) {
		...
		if (check_empty($validation) == true) {
			...
			if ($service['count'] == 0) {
				...
				$total_price = ($service['rows']['price'] / $service['rows']['rate']) * $_POST['quantity'];
				$total_profit = ($service['rows']['profit'] / $service['rows']['rate']) * $_POST['quantity'];
				...
				if ($provider['count'] == 0) {
					...
					$curl = '';
					$provider_order_id = '1';
					if ($service['rows']['provider_id'] == '1') { // PAPADZUL
					    if ($service['rows']['type'] == "Default") {
					        $post_api = array(
                                'api_id' => $provider['rows']['api_id'],
    							'api_key' => $provider['rows']['api_key'],
    							'secret_key' => $provider['rows']['secret_key'],
    							'service' => $service['rows']['provider_service_id'],
    							'target' => $_POST['data'],
    							'quantity' => $_POST['quantity']
    						);
					    } else if ($service['rows']['type'] == "Package") {
					        $post_api = array(
                                'api_id' => $provider['rows']['api_id'],
    							'api_key' => $provider['rows']['api_key'],
    							'secret_key' => $provider['rows']['secret_key'],
						...
						if (isset($result['response']) AND $result['response'] == true) {
							...
							$result_api = true;
						} else if (isset($result['response']) AND $result['response'] == false) {
						    $_SESSION['result'] = array('alert' => 'danger', 'title' => 'Gagal!', 'msg' => ''.$result['data']['msg']);
						}
					}
					if ($result_api == false) {
...
?>
	...
	<div class="form-row">
		<div class="form-group col-md-4">
			<label id="label-price">Harga / 1000</label>
...
function total_price(service, quantity) {
        ...
        var currentPrice = $('#rate').val();
        $.ajax({
            type: "POST",
            url: "<?php echo $config['web']['base_url']; ?>ajax/order-get-price.php",
            data: "service=" + service + "&price=" + currentPrice + "&quantity=" + quantity,
            dataType: "json",
            success: function(data) {
                $('#total-price').val(data.total_price);
            },
            error: function() {
                $('#ajax-result').html('<font color="red">3. Terjadi kesalahan! Silahkan refresh halaman.</font>');
            }
        });
    }
</script>
